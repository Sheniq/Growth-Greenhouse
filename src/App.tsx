...
async function sourceSnapshot(path?: string): Promise<Source> {
  if (!("__TAURI_INTERNALS__" in window)) throw new Error("请使用成长温室 Windows 应用读取 Patina 数据");
  return invoke<Source>("get_patina_source", { databasePath: path || null });
}

async function sessionSnapshot(since: number, until: number, path?: string): Promise<Session[]> {
  if (!("__TAURI_INTERNALS__" in window)) throw new Error("请使用成长温室 Windows 应用读取 Patina 数据");
  return invoke<Session[]>("read_patina_sessions", { sinceMs: since, untilMs: until, databasePath: path || null });
}
...
export default function App() {
  ...
  const selectDatabaseFile = async () => {
    setError("");
    try {
      if (!("__TAURI_INTERNALS__" in window)) throw new Error("请使用成长温室 Windows 应用选择 Patina 数据库");
      const selectedPath = await invoke<string | null>("select_patina_database");
      if (selectedPath) savePath(selectedPath);
    } catch (cause) {
...
function EmptyState({ onClick }: { onClick: () => void }) { const language = useLanguage(); return <div className="empty-state"><div><Sprout size={24} /></div><strong>{text(language, "还没有进行中的目标", "Your greenhouse is empty")}</strong><span>{text(language, "创建一个目标，开始记录第一步。", "Create a goal and leave your first leaf.")}</span><button className="secondary" onClick={onClick}><Plus size={15} /> {text(language, "创建目标", "Create goal")}</button></div>; }
 ...
 type SettingsProps = { path: string; source: Source | null; sessions: Session[]; syncing: boolean; appCategories: AppCategories; manualLearningDrops: number; onCategoryChange: (appKey: string, category: AppCategory) => void; lastSyncedAt: number | null; onSave: (path: string) => void; onFileSelect: () => Promise<void> };
 ...
 function SettingsContent({ path, source, sessions, syncing, appCategories, manualLearningDrops, onCategoryChange, lastSyncedAt, onSave, onFileSelect }: SettingsProps) {
   ...
   return <div className="page-stack settings-page"><div className="page-intro"><span className="section-label">数据源</span><h2>连接 Patina</h2><p>只读读取 Patina 的前台活动记录，用于计算目标进度。</p></div><SoftwareUsage sessions={sessions} syncing={syncing} appCategories={appCategories} manualLearningDrops={manualLearningDrops} onCategoryChange={onCategoryChange} lastSyncedAt={lastSyncedAt} /><section className="panel settings-panel"><div className="path-hints"><div><span>程序路径</span><code>C:\Users\你的用户名\AppData\Local\Patina\Patina.exe</code></div><div><span>数据库路径</span><code>C:\Users\你的用户名\AppData\Roaming\Patina\patina.db</code></div></div><label>数据库路径<span>留空则使用默认路径：C:\Users\你的用户名\AppData\Roaming\Patina\patina.db</span><input value={value} onChange={(event) => setValue(event.target.value)} placeholder="留空使用默认路径" /></label><div className="settings-actions"><button className="primary" onClick={() => onSave(value)}><Check size={15} /> 保存路径</button><button className="secondary" onClick={() => void onFileSelect()}><FolderOpen size={15} /> 选择数据库</button></div>{source?.databasePath ? <code className="selected-path">当前读取：{source.databasePath}</code> : null}</section></div>;
 }
