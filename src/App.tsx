...
import {
  ...
  Languages, Menu, Plus, RefreshCw, Settings2, Sprout, Target, Trophy, Warehouse, X,
} from "lucide-react";
...
const stage = (units: number, completed = false, language: Language = "zh") => completed ? text(language, "已移入花园", "Moved to garden") : units >= 30 ? text(language, "开花结果", "Flowering") : units >= 15 ? text(language, "成熟植物", "Mature plant") : units >= 5 ? text(language, "茁壮幼苗", "Seedling") : units >= 1 ? text(language, "刚刚发芽", "Sprouted") : text(language, "一粒种子", "Seed");
...
export default function App() {
  ...
  const [language, setLanguage] = useState<Language>(() => read(LANGUAGE_KEY, "zh"));
  ...
  useEffect(() => { write(LANGUAGE_KEY, language); }, [language]);
  useEffect(() => {
